...
- `legacy-pages/` - the original standalone `privacy.html` / `support.html` that
  predate the flytunl.ch site. The live versions are
  `flytunl-site/site/privacy/index.html` and `.../support/index.html`; these
  copies are kept only for history and are not deployed or linked anywhere.

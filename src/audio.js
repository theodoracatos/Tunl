...
function _playBgmBuffer() {
    ...
    if (!_bgmGain) {
        ...
        // Sector intensity stage (bgmSetSector): a level lift and a presence shelf.
        _bgmLift  = _ac.createGain();
...
// `chain` (a hazard graze, constants.js GRAZE_* doc) climbs the sweep a step of the
// play track's D-major pentatonic per chain level, starting from A5 (A B D E F#), the
// same idea as sfxCoin's combo ladder. No arg (the wall near-miss) plays the base sweep.
function sfxNearMiss(chain) {
    if (!_ac || !fxOn) return;
    const t = _ac.currentTime;
    const STEPS = [0, 2, 5, 7, 9];   // semitones above A5
    const mul = Math.pow(2, STEPS[Math.min(Math.max((chain | 0) - 1, 0), STEPS.length - 1)] / 12);
    const o = _ac.createOscillator(), g = _ac.createGain();
    o.connect(g); g.connect(_master);
    o.type = 'sine';
    o.frequency.setValueAtTime(880 * mul, t);
    o.frequency.exponentialRampToValueAtTime(1760 * mul, t + 0.10);

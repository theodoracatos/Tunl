...
// links to the store listings), the legal links (#legal, Impressum + privacy
// policy), the portrait gate (#rot), and the run-recording
...
function _fromWebOverlay(e) {
    return !!(e && e.target && e.target.closest && e.target.closest('#cta,#legal,#rot,#rec-btn,#rec-panel'));
}
...
function triggerMilestone(n) {
    ...
    sfxMilestone(n);
    // tunl_ach_score_100000 ("Sechs Stellen") is NOT here -- it's a lifetime-score
    // total now, not a single run's milestone. See the lifetimeScore doc in state.js.
    if (n === 1000 || n === 10000) {
        const _id = n === 1000 ? 'tunl_ach_score_1000' : 'tunl_ach_score_10000';

...
#
#  Usage:  Screenshots/build-portrait-video.sh [source.mp4] [output.mp4] [locale] [canvas]
...
#  e.g. "en" or "pt-BR") and defaults to "en". `canvas` is "portrait" (9:16,
#  the default) or "square" (1:1, the third asset ratio a Google Ads App
#  campaign scores ad strength on); the output canvas size is read back off
#  the rendered frame rather than hardcoded, so adding a ratio needs no
#  change here.
# ============================================================
set -euo pipefail
cd "$(dirname "$0")/.."
...
export TUNL_CANVAS="${4:-portrait}"
if [ "$TUNL_CANVAS" = "square" ]; then
  FRAME="$(dirname "$OUT")/square-video-frame.png"
else
...
# Canvas size comes from the frame we just rendered, not a literal, so the
# same pad works for 1080x2340 and 1080x1080.
read -r OUT_W OUT_H <<<"$(ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 "$FRAME" | tr x ' ')"

echo "Compositing full width of $SRC into the ${OUT_W}x${OUT_H} hole (scale to ${CW}x${CH} at ${X},${Y})..."
ffmpeg -y \
  -i "$SRC" \
  -loop 1 -i "$FRAME" \
  -filter_complex "\
[0:v]scale=${CW}:${CH}:flags=lanczos[vid]; \
[vid]pad=${OUT_W}:${OUT_H}:${X}:${Y}:color=black@0[padded]; \

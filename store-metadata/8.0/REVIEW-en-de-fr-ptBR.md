# 8.0 Store-Texte - Gegenlese (EN / DE / FR + pt-BR Brasilien-Satz)

EN/DE/FR liest du gegen. pt-BR = eigener Brasilien-Satz (Phase 4), kein Translate.

Limits: Apple Name/Subtitle 30, Keywords 100, Promo 170, Desc/News 4000. Play Title 30, Short 80, Full 4000, News 500.
...
**en** (30):
```
Hold to fly. A new cave daily.
```
**de** (30):
```
Halten und fliegen, neu am Tag
```
**fr** (29):
```
Maintiens et vole. Neuve/jour
```
**pt-BR** (29):
```
Segura, sobe. Nova todo dia
```
...
**en** (137):
```
Everyone on Earth flies the exact same cave today. Tomorrow it is gone and a new one takes its place. How far can you get before it does?
```
**de** (135):
```
Die ganze Welt fliegt heute exakt dieselbe Höhle. Morgen ist sie weg und eine neue kommt. Wie weit kommst du, bevor der Tag vorbei ist?
```
**fr** (151):
```
Le monde entier vole aujourd'hui dans la même grotte, au pixel près. Demain elle disparaît et une autre arrive. Jusqu'où iras-tu avant la fin du jour ?
```
**pt-BR** (130):
```
Todo dia uma caverna nova, a mesma pro mundo inteiro. Grátis, leve e joga sem internet. Onde você fica no ranking mundial de hoje?
```
...
**en** (90):
```
one tap,flappy,cave,tunnel,flying,arcade,endless,ghost,leaderboard,reflex,one button,skill
```
**de** (87):
```
einhand,flappy,höhle,tunnel,fliegen,arcade,endlos,geschick,reflex,bestenliste,ein knopf
```
**fr** (88):
```
une main,flappy,grotte,tunnel,voler,arcade,sans fin,fantôme,classement,réflexe,un bouton
```
**pt-BR** (87):
```
um toque,flappy,caverna,tunel,voar,arcade,sem fim,fantasma,ranking,reflexo,offline,leve
```
...
**en** (982):
```
Hold the screen and your ship climbs. Let go and gravity takes over. That is the whole game - and it is harder than it sounds.
...
**de** (1061):
```
Halte den Bildschirm und dein Schiff steigt. Loslassen, und die Schwerkraft übernimmt. Das ist das ganze Spiel - und schwerer, als es klingt.
...
**fr** (1087):
```
Maintiens l'écran et ton vaisseau monte. Relâche, et la gravité reprend le dessus. C'est tout le jeu - et c'est plus dur que ça en a l'air.
...
**pt-BR** (950):
```
Segura a tela e a nave sobe. Solta e a gravidade puxa pra baixo. É isso, o jogo inteiro. E vicia.
...
**en** (413):
```
- Coins redrawn: shield, ammo and magnet coins now have their own shape and size, readable at a glance and colorblind-safe
...
**de** (457):
```
- Münzen neu gezeichnet: Schild-, Munitions- und Magnet-Münze haben jetzt eigene Form und Größe, auf einen Blick erkennbar, auch farbenblind
...
**fr** (454):
```
- Pièces redessinées : bouclier, munitions et aimant ont chacun leur forme et leur taille, lisibles d'un coup d'oeil, même daltonien
...
**pt-BR** (482):
```
- Moedas redesenhadas: escudo, munição e ímã agora têm forma e tamanho próprios, dá pra bater o olho e saber qual é, mesmo pra daltônicos
...
**en** (78):
```
Hold to fly through a deadly cave. A new one every day, the same for everyone.
```
**de** (70):
```
Halten, durch die tödliche Höhle fliegen. Neu am Tag, für alle gleich.
```
**fr** (77):
```
Maintiens et vole dans une grotte mortelle. Une neuve chaque jour, pour tous.
```
**pt-BR** (80):
```
Segura pra subir e voa por uma caverna mortal. Nova todo dia, a mesma pro mundo.
```
...
**en** (1241):
```
TUNL is a one-tap cave flyer with a twist: there is only one cave per day, and the whole world plays it.
...
**de** (1389):
```
TUNL ist ein Ein-Tipp-Höhlenflieger mit einem Kniff: Es gibt nur eine Höhle pro Tag, und die ganze Welt spielt sie.
...
**fr** (1440):
```
TUNL est un jeu de vol en grotte à une touche, avec une idée : il n'y a qu'une grotte par jour, et le monde entier y joue.
...
**pt-BR** (1362):
```
TUNL é um jogo de voar em caverna com um toque só. Grátis, leve e joga sem internet. E tem uma ideia diferente: só existe uma caverna por dia, e o mundo inteiro joga ela.
...
### What's New  (Limit 500)

**en** (390):
```
- Coins redrawn: shield, ammo and magnet coins each have their own shape and size now - easy to tell apart at a glance, even colorblind
...
**de** (441):
```
- Münzen neu gezeichnet: Schild-, Munitions- und Magnet-Münze haben jetzt eigene Form und Größe - auf einen Blick zu unterscheiden, auch farbenblind
...
**fr** (437):
```
- Pièces redessinées : bouclier, munitions et aimant ont chacun leur forme et leur taille - reconnaissables d'un coup d'oeil, même daltonien
